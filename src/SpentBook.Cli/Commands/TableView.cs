...
namespace SpentBook.Cli.Commands
{
    ...
    public class TableView
    {
        ...
        public static TableView ToTableView<T>(IEnumerable<T> list, ITableConfig<T> tableConfig, bool wide, int colWidth = 0)
        {
            ...
            {
                table.AddColumnDefinition(col.Name ?? col.Property.Name.ToUpper(), colWidth, 0, 3);
            }
            ...
            foreach (var obj in list)
            {
                ...
                {
                    var value = prop.ValueFormatter(obj).Trim() ?? "";
        ...
        public static TableView ToTableView<T>(IEnumerable<T> list, int colWidth = 0)
        {
            ...
            foreach (var obj in list)
            {
                ...
                foreach (var prop in properties)
                {
                    var value = prop.GetValue(obj)?.ToString().Trim();
                    row.AddColumnInRow(value ?? "");
                }
            }
